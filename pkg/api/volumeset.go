...
import (
	...
	"github.com/controlplane-com/types-go/pkg/volumeSet"
)
...
func (s *VolumeSetService) ListPage(ctx context.Context, org, gvc, cursor string) (*ListResponse[volumeSet.VolumeSet], error) {
	...
	return listPage[volumeSet.VolumeSet](ctx, s.client, path)
}
...
func (s *VolumeSetService) Query(ctx context.Context, org, gvc string, q *query.Query) iter.Seq2[volumeSet.VolumeSet, error] {
	org = s.client.resolveOrg(org)
	path := fmt.Sprintf("/org/%s/gvc/%s/volumeset/-query", org, gvc)
	return queryIterator[volumeSet.VolumeSet](ctx, s.client, path, q)
}

// QueryAll returns all volume sets matching the query.
func (s *VolumeSetService) QueryAll(ctx context.Context, org, gvc string, q *query.Query) ([]volumeSet.VolumeSet, error) {
	org = s.client.resolveOrg(org)
	path := fmt.Sprintf("/org/%s/gvc/%s/volumeset/-query", org, gvc)
...
func (s *VolumeSetService) QueryPage(ctx context.Context, org, gvc string, q *query.Query) (*QueryResponse[volumeSet.VolumeSet], error) {
	org = s.client.resolveOrg(org)
	path := fmt.Sprintf("/org/%s/gvc/%s/volumeset/-query", org, gvc)
	return queryPage[volumeSet.VolumeSet](ctx, s.client, path, q)
}
