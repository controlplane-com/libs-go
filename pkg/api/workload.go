...
func (s *WorkloadService) ListPage(ctx context.Context, org, gvc, cursor string) (*ListResponse[workload.Workload], error) {
	...
	return listPageWithClient[workload.Workload](ctx, s.client, path)
}
...
func (s *WorkloadService) QueryPage(ctx context.Context, org string, q *query.Query) (*QueryResponse[workload.Workload], error) {
	org = s.client.resolveOrg(org)
	path := fmt.Sprintf("/org/%s/workload/-query", org)
	return queryPageWithClient[workload.Workload](ctx, s.client, path, q)
}
...
func (s *WorkloadService) ForceRedeploy(ctx context.Context, org, gvc, name string) error {
	// Fetch current workload to get the version and full spec
	current, err := s.Get(ctx, org, gvc, name)
	if err != nil {
		return err
	}

	// Set the deploy timestamp tag to force a redeploy
	if current.Tags == nil {
		current.Tags = workload.WorkloadTags{}
	}
	current.Tags["cpln/deployTimestamp"] = time.Now().UTC().Format(time.RFC3339Nano)

	// Update with the full workload (including the new tag)
	_, err = s.Update(ctx, org, gvc, name, current)
	return err
}
...
func (s *WorkloadService) CronStart(ctx context.Context, org, gvc, name string, req *CronStartRequest) (*command.Command, error) {
	spec := &command.RunCronWorkloadSpec{
		Location:           req.Location,
		ContainerOverrides: req.ContainerOverrides,
	}
	return s.RunCronWorkload(ctx, org, gvc, name, spec)
}
